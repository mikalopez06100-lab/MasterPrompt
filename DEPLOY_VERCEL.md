...
| `NEXT_PUBLIC_LOGO_URL` | *(Obsolète pour le bandeau et l’OG racine.)* Le site utilise `public/logo.png`. Tu peux supprimer cette variable sur Vercel si elle pointait vers une mauvaise image. | Non | — |
...
- **Logo :** fichier unique `public/logo.png` (bandeau + OG racine). Script optionnel : `node scripts/upload-logo-to-supabase.mjs` pour héberger une copie sur Supabase ; **ne pas** renseigner d’URL erronée dans les variables d’environnement.
